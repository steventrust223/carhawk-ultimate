...
function openAppointmentManager() {
  const html = HtmlService.createTemplateFromFile('AppointmentManager')
    .evaluate()
    .setWidth(800)
    .setHeight(600);

  SpreadsheetApp.getUi().showModalDialog(html, '📅 Appointment Manager');
}

function openFollowUpCenter() {
  const html = HtmlService.createTemplateFromFile('FollowUpCenter')
    .evaluate()
    .setWidth(800)
    .setHeight(600);

  SpreadsheetApp.getUi().showModalDialog(html, '🔄 Follow-up Center');
}

function openCampaignManager() {
  const html = HtmlService.createTemplateFromFile('CampaignManager')
    .evaluate()
    .setWidth(800)
    .setHeight(600);

  SpreadsheetApp.getUi().showModalDialog(html, '📧 Campaign Manager');
}

function openSMSConversations() {
  const html = HtmlService.createTemplateFromFile('SMSConversations')
    .evaluate()
    .setWidth(800)
    .setHeight(600);

  SpreadsheetApp.getUi().showModalDialog(html, '💬 SMS Conversations');
}

function openCallLogs() {
  const html = HtmlService.createTemplateFromFile('CallLogs')
    .evaluate()
    .setWidth(800)
    .setHeight(600);

  SpreadsheetApp.getUi().showModalDialog(html, '📞 AI Call Logs');
}

function openCRMAnalytics() {
  const html = HtmlService.createTemplateFromFile('CRMAnalytics')
    .evaluate()
    .setWidth(1000)
    .setHeight(700);

  SpreadsheetApp.getUi().showModalDialog(html, '📊 CRM Analytics');
}

function openLeadTracker() {
  SpreadsheetApp.getActiveSpreadsheet()
    .setActiveSheet(getQuantumSheet(QUANTUM_SHEETS.LEADS.name));
}

function openLeadScoring() {
  SpreadsheetApp.getActiveSpreadsheet()
    .setActiveSheet(getQuantumSheet(QUANTUM_SHEETS.SCORING.name));
}
